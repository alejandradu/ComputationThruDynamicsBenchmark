...
SEARCH_SPACE = dict(
    ...
    trainer=dict(
        ...
        log_every_n_steps=4,            # steps != epochs
    ),
    
    # configs for dataset (timeseries trials) creation
    params=dict(
        # seed=0,
        batch_size=tune.grid_search([750]),   
        n_samples=2250,                   # number of trials to simulate
        num_workers=tune.choice([2]),  # for the DataLoader. != Ray workers. Usually set = CPUs
    ),
    # TODO: specify configs for datamodule vs env for sim and train
