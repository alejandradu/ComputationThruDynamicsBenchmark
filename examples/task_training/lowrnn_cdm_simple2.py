...
MODEL = "LowRankRNN2"
TASK = "MultiTask"
task_detail = "CDM12_simple_LONG"   
NUM_HYPERPARAM_SAMPLES = 1
RESOURCES_PER_TRIAL = {"CPU": 1, "GPU": 0.2}  # around 0.25 gpu per node_cdm trial
# TUNE_SEARCH_ALG=
# TUNE_SCHEDULER=
...
SEARCH_SPACE = dict(
    ...
    task_wrapper=dict(
        weight_decay=1e-9,
        learning_rate=tune.grid_search([5e-2, 1e-3]),    # 1e-2 causes exploding gradient eventually
    ),
    ...
    params=dict(
        ...
        num_workers=5,  # for the DataLoader. != Ray workers. Usually set = CPUs
    ),
