...
MODEL = "NODE"
TASK = "MultiTask"
task_detail = "CDM12_LAT10_2"   
NUM_HYPERPARAM_SAMPLES = 1
RESOURCES_PER_TRIAL = {"CPU": 1, "GPU": 0.33}  # around 0.25 gpu per node_cdm trial
# TUNE_SEARCH_ALG=
# TUNE_SCHEDULER=
...
SEARCH_SPACE = dict(
    ...
    task_wrapper=dict(
        weight_decay=1e-9,
        learning_rate=1e-3,    # 1e-2 causes exploding gradient eventually
    ),
    ...
    params=dict(
        ...
        num_workers=3,  # for the DataLoader. != Ray workers. Usually set = CPUs
    ),
    ...
    # will be too large
    env_task=dict(
        task_list=["ContextIntMod1", "ContextIntMod2"],    # if using choice remember to use double brackets [[...]]
        # latent_l2_wt=tune.grid_search([1e-6, 1e-3]),
        # n_timesteps=1000,
        noise=tune.grid_search([0, 0.01, 0.02])
        # dynamic_noise: 0.0    # zero by default
        # num_targets: 32
        # bin_size: 20
    ...
    env_sim=dict(
        task_list=["ContextIntMod1", "ContextIntMod2"],
        #latent_l2_wt=tune.grid_search([1e-6, 1e-3]),
        # n_timesteps: 320
        noise=0.0,
        dynamic_noise=0.1,
